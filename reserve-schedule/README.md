...
2. **通知先メールアドレス等の管理画面「設定」タブ → 実装済み。** `admin.html` の「設定」タブから通知先メール（複数行）・営業時間・受付締切（時間単位）・予約可能日数（日単位）を変更・保存できる。あわせて、予約可能範囲の設定粒度を「ヶ月」から「日数」に変更した（`BOOKING_CONFIG.MAX_ADVANCE_DAYS`、旧`MAX_ADVANCE_MONTHS`から置き換え）。
   お客様側の連絡先修正（`index.html` の「予約の確認・変更」パネル）も実装済み。予約番号＋電話番号で本人確認のうえ、電話番号・メールアドレスを修正できる。
...
- メニューの時間帯制約:「全身すっきり！あかすり＋全身保湿ケア」は **12時〜20時限定**。
  → **現在の実装は「開始時刻が12:00〜20:00の範囲内であればよい」という解釈**（`startWindow.min=12*60, max=20*60`）で実装しています。120分メニューのため、20:00開始の場合は施術終了が22:00になります。オーナーへの確認待ち。もし「20時までに施術が終了している必要がある」（＝最終開始時刻は18:00）という意味であれば、`shared.js` の `MENU_ITEMS` 内 `akasuri120.startWindow.max` を `20*60` から `18*60` に修正してください（下記(D)-4も参照）。
...
4. **「あかすり 12時〜20時限定」の解釈確認待ち。**
   - 現在の実装は「**開始時刻が12:00〜20:00の範囲内であればよい**」という解釈（`startWindow = {min:12*60, max:20*60}`）。120分メニューなので20:00開始を選ぶと施術終了は22:00になる。
   - もしオーナーの意図が「**20時までに施術が終了している必要がある**」（＝最終開始時刻は18:00）であれば、`shared.js` の `MENU_ITEMS` 内 `akasuri120` の `startWindow.max` を `20*60` から `18*60` に修正すること。
   - 該当コード: `reserve-schedule/shared.js` の `MENU_ITEMS` 配列、`id: "akasuri120"` の `startWindow` フィールド。
