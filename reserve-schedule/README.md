...
- メニューの時間帯制約:「全身すっきり！あかすり＋全身保湿ケア」は **12時〜20時限定**。
  → **現在の実装は「20時までに施術が終了する開始時刻のみ選択可（＝最終開始時刻は18:00）」という解釈**で実装しています。オーナーへの確認待ち。「20時までに開始すればよい」という意味であれば `shared.js` の `MENU_ITEMS` 内 `akasuri120.startWindow.max` を `20*60` から `18*60`（現状の意図での終了ベース）または別の値に修正してください（下記(D)-4も参照）。
...
4. **「あかすり 12時〜20時限定」の解釈確認待ち。**
   - 現在の実装は「**20時までに施術が終了する開始時刻のみ選択可**」＝120分メニューなので最終開始時刻は 18:00、という解釈。
   - もしオーナーの意図が「**20時までに開始すればよい**」（＝最終開始時刻 20:00、終了は22:00になってもよい）であれば、`shared.js` の `MENU_ITEMS` 内 `akasuri120` の `startWindow.max` を修正すること。
